...
Namespace GradientHeightMap
    Public Class HeightMapViewModel
        ...
        Private Sub GenerateMap(ByVal uri As Uri)
            ...
            Dim imageData_Renamed As New ImageData(New DoubleCollection(countX), New DoubleCollection(countZ), New DoubleCollection(countX * countZ))
            Dim fullZ As Boolean = False
            For i As Integer = 0 To countX - 1
                imageData_Renamed.XArguments.Add(startX + i * gridStep)
                For j As Integer = 0 To countZ - 1
                    If Not fullZ Then
                        imageData_Renamed.YArguments.Add(startZ + j * gridStep)
                    End If
                    Dim value As Double = GetValue(pixels(i, j), minY, maxY)
                    imageData_Renamed.Values.Add(value)
                Next j
                fullZ = True
            Next i
            ImageData = imageData_Renamed
        End Sub
